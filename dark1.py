...
# Extend argument parsing for number of exchanges
...
parser.add_argument('--exchanges', type=int, default=1, help="Number of intra-team exchanges of articles.")
...
def validate_treaty_outcome(treaty_outline, goals=treaty_goals, accomplishments=treaty_accomplishments):
    ...
    if set(goals).issubset(set(treaty_outline.values())) and set(accomplishments).issubset(set(treaty_outline.values())):
        print("Treaty article meets all predefined goals and accomplishments.")
        return True
    else:
        print("Treaty article does not meet all predefined goals and accomplishments. Further negotiation needed.")
        return False
...
def start_negotiation_with_structure(manager, reference_treaties):
    ...
    for article_id, scope in agreed_structure.items():
        for exchange in range(num_exchanges):  # Track exchange number for printing
            print(f"{Colors.YELLOW}Exchange {exchange + 1} for Article {article_id}: {scope}{Colors.ENDC}")  # Highlight exchange number and article being negotiated
            for agent in us_agents + russian_agents:
                response = negotiate_article(agent, article_id, scope)
                # Assuming `negotiate_article` returns the draft in a format that includes the article text
                print(f"{Colors.YELLOW}{response}{Colors.ENDC}")  # Print the draft in yellow

    # After all exchanges for each article, compile Articles into a document
    compile_articles_to_document(all_agents, reference_treaties)
